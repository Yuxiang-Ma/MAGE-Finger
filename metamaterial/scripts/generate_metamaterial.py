...
def main() -> None:
    ...
    parser.add_argument(
        "--unit-cell-size", "-u", type=float, default=5.0, help="Unit cell size in mm"
    )
    parser.add_argument(
        "--density",
        type=float,
    ...
    parser.add_argument(
        "--material", default="85A", help="TPU grade for stiffness report"
    )
    parser.add_argument(
        "--wall-layers",
        type=int,
    ...
    if args.output is None:
        ...
        dens_tag = (
            ...
            else (f"off{args.offset:.2g}" if args.offset is not None else "d0.3")
        )
    ...
    if out.n_cells == 0:
        print(
            "[error] No mesh generated - try a different density/cell size.",
            file=sys.stderr,
        )
        sys.exit(1)
