...
Checks performed:
  1. Open edges          — CRITICAL: must be 0 for a watertight mesh
  ...
  5. Tiny edges          — WARNING:  features smaller than nozzle cannot print
...
class InspectionReport:
    ...
    def print(self, verbose: bool = False) -> None:
        COLORS = {"PASS": "\033[92m", "WARN": "\033[93m", "FAIL": "\033[91m", "INFO": "\033[96m"}
        RESET = "\033[0m"
...
def check_manifold(mesh: pv.PolyData, report: InspectionReport) -> None:
    if mesh.is_manifold:
        ...
        # Count non-manifold edges separately
...
def check_degenerate_faces(mesh: pv.PolyData, report: InspectionReport) -> None:
    ...
    if n_degen == 0:
        ...
        report.add("Degenerate faces", "WARN",
...
def check_feature_size(
    ...
    """Estimate mean wall thickness via the hydraulic chord length 2·V / A.

    TPMS meshes have sub-nozzle tessellation edges (0.1–0.3 mm) that are mesh
    artefacts unrelated to physical strut size.  Edge-length statistics are
    therefore unreliable proxies.

    The hydraulic chord length 2V/A is the mean distance a random ray travels
    through solid material and scales correctly with unit-cell size:
      - 3 mm gyroid ≈ 0.88 mm wall thickness
      - 5 mm gyroid ≈ 1.38 mm wall thickness
      - 8 mm gyroid ≈ 2.01 mm wall thickness

    This metric works for both PyScaffolder output (fine tessellation) and
    custom marching-cubes output (coarser tessellation).
    """
    ...
    thickness = 2.0 * vol / area  # mean wall thickness in mm

    detail = (
        f"Solid volume: {vol:.1f} mm³  |  Surface area: {area:.1f} mm²\n"
    ...
    if thickness >= min_feature_mm:
        ...
        report.add("Feature size", "WARN",
                   f"Mean wall {thickness:.2f} mm — struts may be marginal for rigid materials",
                   detail + "\nTPU can print thinner features than rigid filaments; likely OK.")
    else:
...
def check_normals(mesh: pv.PolyData, report: InspectionReport) -> None:
    """Check normal consistency using pyvista's auto-orient pass.

    Centroid-based outward tests are unreliable for non-convex lattices (TPMS)
    because faces legitimately point toward the centroid on interior struts.
    Instead, we verify that pyvista can consistently orient all normals outward
    via propagation across shared edges.  Failure indicates disconnected or
    degenerate topology that the manifold check may have missed.
    """
...
def check_bounds(mesh: pv.PolyData, report: InspectionReport, build_plate: tuple) -> None:
    ...
    msg = (
        f"Model: {size[0]:.1f}×{size[1]:.1f}×{size[2]:.1f} mm  "
        f"Build plate: {build_plate[0]}×{build_plate[1]}×{build_plate[2]} mm"
    )
    if fits:
        report.add("Build volume", "INFO", f"Fits Bambu build plate  ({msg})")
    else:
        report.add("Build volume", "WARN", f"Exceeds build plate!  ({msg})")
...
def main() -> None:
    ...
    parser.add_argument("--min-feature", type=float, default=0.8,
                        help="Minimum recommended feature size in mm (2× nozzle is typical)")
