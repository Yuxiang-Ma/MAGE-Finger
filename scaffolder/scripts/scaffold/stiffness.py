...
def stiffness_report(
    ...
    """
    iso_b = iso_for_spring_constant(
        k_base, cross_section_mm2, thickness_mm, material, surface
    )
    iso_t = iso_for_spring_constant(
        k_tip, cross_section_mm2, thickness_mm, material, surface
    )
...
def print_stiffness_report(report: dict) -> None:
    print(f"\n{'=' * 52}")
    print("  Stiffness Design Report")
    print(f"{'=' * 52}")
    print(
        f"  Base (stiff end): {report['k_base_NmM']:.2f} N/mm  ->  iso {report['iso_base']:+.3f}  "
        f"({report['solid_fraction_base']:.0%} solid)"
    )
    print(
        f"  Tip  (soft end) : {report['k_tip_NmM']:.2f} N/mm  ->  iso {report['iso_tip']:+.3f}  "
        f"({report['solid_fraction_tip']:.0%} solid)"
    )
    print(
        f"  Stiffness ratio : {1 / report['stiffness_ratio']:.1f}x  "
        f"(E_rel Gibson-Ashby = {report['E_rel_GA']:.3f})"
    )
    print(
        f"  Gradient args   : --isolevel-start {report['gradient_args']['--isolevel-start']} "
